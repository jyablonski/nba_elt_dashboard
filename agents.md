...
| `pandas`                             | In-memory table snapshots served through `get_table`    |
...
- **`src/server.py`** constructs `dash.Dash`, registers **`dbc.Tabs`** as the main shell (no separate top brand row; **`app.title`** still sets the browser tab title), and builds each tab’s `children=` from layout functions imported from **`src/pages/`**.
- **Navigation:** single-page app with tabs (not multipage URLs). Tab counts/badges are built in the dynamic layout path using `src.shell` helpers.
...
**`src/data_access/cache.py`** runs an initial load at **import time**:

1. Loads **`config.yaml`** via `src.data_access.database.load_yaml_with_env` keyed by **`ENV_TYPE`** (default `dev`).
2. Opens a SQLAlchemy engine with **`src.data_access.database.create_dashboard_engine`**.
3. Calls **`refresh_data`**, which loops **`src.data_access.tables.source_tables`** and publishes DataFrames into a thread-safe in-memory snapshot.

**Implications:** Importing `src.server` or `src.data_access.cache` pulls the full dataset into memory and **requires a live DB** unless tests use mocks/Testcontainers. Page layouts and callbacks should read data with **`src.data_access.cache.get_table`** so the dashboard can use refreshed snapshots without restarting.

Supporting modules: **`src/data_access/database.py`** (YAML/env config, engine factory, configured engine, and table reads), **`src/data_access/tables.py`** (table list and team constants), **`src/config.py`** (shared app constants / re-exports where noted).
...
src/
  ...
  data_access/       # DB config, engine factory, queries, table list, refresh cache
  ...
  table_columns/     # Dash DataTable column specs per domain table
static/
...
**`src/table_columns/*.py`** holds `list[dict]` column definitions for `dash_table.DataTable` (often with `FormatTemplate` / `Format`).

## Configuration

- **`config.yaml`** - per-environment host, port, credentials, schema. **`ENV_TYPE`** selects the block; values can use **`${VAR}`** env substitution (see `src.data_access.database`).
...
| `make test`                        | Full pytest + coverage (≥95% on measured `src/`; **`src/pages/*` omitted** from coverage denominator in `pyproject.toml`) |
...
- **New data surfaces:** add the table to **`src.data_access.tables.source_tables`**, consume it with **`src.data_access.cache.get_table("your_table")`**, and add **`src/table_columns/`** specs if tabular.
